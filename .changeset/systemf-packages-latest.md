...
Effect moves to `4.0.0-rc.117`, along with `@effect/platform-node`, `@effect/platform-node-shared`, `@effect/vitest` and `@effect/opentelemetry`. Projects that install `effect` next to these packages need the same release.
