...
| **E2E-4** | Each fixture declares every plugin it loads in its own `stryker.config.json`. There is no plugin glob: that array is the only source of what loads.                                                                                                                                          | `review` — every `testResources/*/stryker.config.json` names each plugin its run loads                            |
...
| `OTEL_SERVICE_NAME`              | default `stryker-js-cli-e2e`                                      | One service name across the CLI, its workers and the test process — what the journey searches |
