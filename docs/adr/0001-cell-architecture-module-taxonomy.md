...
- **Tests.** Workflows get properties in `src/__tests__/*.property.test.ts`. Cells, handles, services, and the composition root get no unit tests of their own: the integration suites, the e2e lane, and the api-extractor reports grade them.
