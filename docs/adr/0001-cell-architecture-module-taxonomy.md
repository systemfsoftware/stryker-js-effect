...
- **Tests.** Properties under `src/` live in `src/__tests__/<stem>.workflow.property.test.ts`, the only basename `test-discipline` accepts there, and `<stem>` names the module the laws grade. Workflows get properties. Cells, services, and the composition root get no unit tests of their own: the integration suites, the e2e lane, and the api-extractor reports grade them. Two cases get laws outside a workflow, each only when an independent oracle pins consumer-visible behavior:
  ...
  - A pure helper no workflow can host. `make-body-purity` refuses a reference to a local module or to any dependency outside the sealed `effect/*` surface, and workflow bodies are held to complexity 1, so such a helper keeps its laws in an in-source `import.meta.vitest` block in the module that owns it. `Survivors.cell.ts`'s `hashContent` (it needs `@noble/hashes`) keeps only `∀kv_HashContent_≡KnownAnswerVectors`, because a prior report's fingerprints are a published input, and no laws that only restate the hashing library. `conclude-run.cell.ts` owns the cause walk that turns the command handler's `Exit` into `RunOutcomeCommand` (it reads `CliError`, `ErrorText`, and survivors refusals, and branches on every cause node), so its laws sit beside it and `RunOutcomeCommand.schema.ts` stays declaration-only.
