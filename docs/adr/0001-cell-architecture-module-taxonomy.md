...
- **Tests.** Properties under `src/` live in `src/__tests__/<stem>.workflow.property.test.ts`, the only basename `test-discipline` accepts there, and `<stem>` names the module the laws grade. Workflows get properties. Cells, services, and the composition root get no unit tests of their own: the integration suites, the e2e lane, and the api-extractor reports grade them. Two cases get laws outside a workflow, each only when an independent oracle pins consumer-visible behavior:
  ...
  - A pure helper no workflow can host. `make-body-purity` refuses a reference to a local module or to any dependency outside the sealed `effect/*` surface, and workflow bodies are held to complexity 1, so such a helper keeps its laws in an in-source `import.meta.vitest` block in the module that owns it. `conclude-run.cell.ts` owns the cause walk that turns the command handler's `Exit` into `RunOutcomeCommand` (it reads `CliError`, `ErrorText`, and survivors refusals, and branches on every cause node), so its laws sit beside it and `RunOutcomeCommand.schema.ts` stays declaration-only. A helper that only wraps a library gets no laws: `Survivors.cell.ts`'s `hashContent` hashes both sides of the survivors comparison in the same process, so no digest crosses a version boundary, and known-answer vectors would only restate `@noble/hashes`.
  - A refined schema's refusal boundary. The generated `schema-laws.test.ts` laws only prove that accepted values round-trip, so every schema carrying a check, bound, pattern, or struct-level relation states what it refuses in an in-source block in its own `*.schema.ts`. The law draws the unrefined base (`S.Int`, `S.String`), appends boundary values (`-1`, `0`, the maximum, `''`) to every draw, and compares acceptance with a predicate written from the domain contract. `Location.schema.ts` is the reference. Functions over a schema's data live in an unsuffixed sibling module named after the type (`Location.ts`), never in the schema file.
- **A property draws its input.** A behavior law generates its input from a Schema-derived arbitrary over the input's real domain and checks it against an oracle that does not re-derive the answer the way the code does. A constant as the whole input, a hand-picked list of example values, or a known-answer table is an example test with a property label and is deleted. A constant may still pin one field of a generated record, and a closed literal union may be drawn when it is the whole domain. A struct-level relation whose schema appears inside collections generates through `S.declare` with a `toCodecArbitrary` that builds valid values, because rejection sampling exhausts the generated round-trip laws.
