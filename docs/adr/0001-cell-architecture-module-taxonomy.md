...
- **Tests.** Workflows get properties in `src/__tests__/<stem>.workflow.property.test.ts`; `test-discipline` accepts no other property-test basename under `src/`. Cells, handles, services, and the composition root get no unit tests of their own: the integration suites, the e2e lane, and the api-extractor reports grade them. One case the same gate carves out, and this record names it: a pure helper no workflow can host — `make-body-purity` refuses a reference to a local module or to any dependency outside the sealed `effect/*` surface, which is why `Survivors.cell.ts`'s `hashContent` (it needs `@noble/hashes`) has no workflow home — keeps its laws in an in-source `import.meta.vitest` block in the module that owns it. That block holds only laws pinning the module's own consumer contract: `hashContent` keeps `∀kv_HashContent_≡KnownAnswerVectors`, because a prior report's fingerprints are a published input, and not laws that only restate the hashing library.
