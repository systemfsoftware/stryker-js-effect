...
`scripts/publish-and-setup-npm-trust.ts` (run via `pnpm publish:unpublished
...
1. **Manifest carries the debut `version`**, and `repository.url` exactly
   `git+https://github.com/systemfsoftware/stryker-js-effect.git` with the
...
2. **`corepack pnpm --filter <pkg> build`** — mandatory. Most packages
   ...
   states this at `.github/workflows/release.yml:85-88`).
...
4. **`npm trust github <pkg> --repo systemfsoftware/stryker-js-effect --file release.yml --allow-publish --yes`**
5. **`npm trust list <pkg>`** to confirm the GitHub Actions publisher is
   registered against `systemfsoftware/stryker-js-effect` + `release.yml`.
...
then one block per never-published package with its three bootstrap commands,
and exited 1.
...
`npm trust github <pkg> --repo systemfsoftware/stryker-js-effect --file release.yml --allow-publish --yes` →
