...
const isObjectExpression = (node: Node): boolean => node.type === 'ObjectExpression'
...
const plainMemberOf = (node: Node, objectNames: readonly string[]): MemberExpression | undefined =>
  isPlainMemberOf(node, objectNames) ? node : undefined

const hasNamedMember = (node: Node, objectNames: readonly string[], propertyName: string): boolean => {
...
const signalIoArgumentIndex = (callee: Node): number | undefined =>
  SIGNAL_IO_ARGUMENT_MATCHERS.find(([, matches]) => matches(callee))?.[0]

const isSignalQueryCall = (callee: Node): boolean => SIGNAL_QUERY_CALL_MATCHERS.some((matches) => matches(callee))

const ownsPropertyDefinitionField = (owner: Node | undefined): boolean => owner?.type === 'PropertyDefinition'

const ownsClassField = (owner: Node | undefined): boolean => CLASS_FIELD_KINDS.some((kind) => kind === owner?.type)

type OwnsCallSite = (owner: Node | undefined) => boolean

const holdsArgument = (call: CallExpression, node: Node): boolean =>
  call.arguments.some((argument: unknown) => argument === node)

const isObjectArgumentOf = (node: Node, call: CallExpression): boolean =>
  isObjectExpression(node) && holdsArgument(call, node)

const parentCallOf = (ancestors: readonly Node[], offset: number): CallExpression | undefined => {
  const parent = ancestors[offset]
  return isCallExpression(parent) ? parent : undefined
}

const callOfObjectArgument = (node: Node, call: CallExpression): CallExpression | undefined =>
  isObjectArgumentOf(node, call) ? call : undefined

const argumentCallOf = (node: Node, ancestors: readonly Node[], offset: number): CallExpression | undefined => {
...
const isArgumentAt = (index: number | undefined, call: CallExpression, node: Node): boolean =>
  index !== undefined && call.arguments[index] === node

const ioConfigReason = (call: CallExpression, node: Node): string | undefined =>
  isArgumentAt(signalIoArgumentIndex(call.callee), call, node) ? INPUT_MODEL_OUTPUT_CONFIG_MSG : undefined

const isQueryOptionsArgument = (call: CallExpression, node: Node): boolean =>
  isSignalQueryCall(call.callee) && isArgumentAt(1, call, node)

const queryOptionsReason = (call: CallExpression, node: Node): string | undefined =>
  isQueryOptionsArgument(call, node) ? SIGNAL_QUERY_OPTIONS_MSG : undefined

const ioReasonOf = (node: Node, ancestors: readonly Node[], offset: number): string | undefined => {
...
const queryReasonOf = (node: Node, ancestors: readonly Node[], offset: number): string | undefined => {
  const call = ownedCallOf(node, ancestors, offset, ownsClassField)
  return call === undefined ? undefined : queryOptionsReason(call, node)
}

const reasonAt = (node: Node, ancestors: readonly Node[], offset: number): string | undefined =>
  ioReasonOf(node, ancestors, offset) ?? queryReasonOf(node, ancestors, offset)

const ancestorReason = (node: Node, ancestors: readonly Node[]): string | undefined =>
