...
Feature('Effect Schema declarations — ignored mutants on tags, brands, optionals, and annotation objects')
  .body(({ scenario }) => {
    ...
    scenario(
      'The exported descriptor registers under the schema-declarations name and answers like the decision function',
      Gherkin.Do.pipe(
        Given('a Symbol.for description under its enclosing call, and the package descriptor')(
          'node',
          () =>
            Effect.sync(() => {
              const description = stringLiteral('MyBrand')
              const call = symbolForCall(description)
              const path = { node: description, parentPath: { node: call, parentPath: null } }
              return { path, description, call, descriptor: strykerIgnorers[0] }
            }),
        ),
        When('the descriptor and the decision function each examine it')('answers', (s) =>
          Effect.sync(() => ({
            ...
            decision: decideSchemaDeclarationIgnore(s.node.description, s.node.call),
          }))),
        Then('the descriptor carries the registered name and both answer with the symbol-description reason')((s) =>
          Effect.sync(() => {
            expect(s.answers.name).toBe('effect-schema-declarations')
            expect(s.answers.descriptor).toBe(SYMBOL_DESCRIPTION_IGNORED)
