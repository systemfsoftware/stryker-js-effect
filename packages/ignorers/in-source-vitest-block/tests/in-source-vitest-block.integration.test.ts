...
import {
  ...
  isInSourceTestGuard,
  strykerIgnorers,
} from '@systemfsoftware/stryker-ignorer-in-source-vitest-block'
...
Feature('In-source Vitest test guard — the if-statement shape and the ancestor walk')
  .body(({ scenario }) => {
    ...
    scenario(
      ...
      Gherkin.Do.pipe(
        Given(
          'a mutant whose only ancestor is an `if (import.meta.vitest)` guard, and the package descriptor',
        )('node', () =>
          Effect.sync(() => {
            const guard = guardOf(importMetaMember('vitest'))
            const path = { node: identifier('x'), parentPath: { node: guard, parentPath: null } }
            return { path, ancestors: [guard], descriptor: strykerIgnorers[0] }
          })),
        When('the descriptor and the decision function each examine it')('answers', (s) =>
          Effect.sync(() => ({
            ...
            decision: decideInSourceTestIgnore(s.node.ancestors),
          }))),
        Then('the descriptor carries the registered name and both answer with IN_SOURCE_TEST_IGNORED')((s) =>
          Effect.sync(() => {
            expect(s.answers.name).toBe('in-source-vitest-block')
            expect(s.answers.descriptor).toBe(IN_SOURCE_TEST_IGNORED)
