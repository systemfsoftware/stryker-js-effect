...
import { CauseText, type ErrnoException, ErrorText } from './ErrorText.schema.js'

const nonEmptyText = Option.liftPredicate(S.is(S.NonEmptyString))

export const errorTextOf = <A = unknown>(cause: A): Option.Option<ErrorText> =>
  Option.flatMap(nonEmptyText(errorTextValueOf(cause)), (text) => ErrorText.makeOption({ text }))

export const causeTextOf = <A = unknown>(cause: A): Option.Option<CauseText> =>
  Option.flatMap(nonEmptyText(causeChainTextOf(cause)), (text) => CauseText.makeOption({ text }))

function errorTextValueOf<A = unknown>(error: A): string {
...
function missingCauseTextOf<A = unknown>(cause: A, depth: number): string | undefined {
  return Boolean.match(isMissingCause(cause), {
    onTrue: () => undefined,
    onFalse: () => causeNodeTextOf(cause, depth),
  })
}

function causeNodeTextOf<A = unknown>(cause: A, depth: number): string | undefined {
