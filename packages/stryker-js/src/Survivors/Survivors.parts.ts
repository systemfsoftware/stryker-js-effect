...
export const resolveSurvivorsRunOptions = (input: {
  readonly cliOptions: Options.PartialStrykerOptions
  readonly mode: OutputMode
}) => readConfig(input.cliOptions, { command: 'run', mode: input.mode })
...
export const readPriorReport = (priorReportPath: string) =>
...
export const currentSourceHashesFor = (files: readonly string[]) =>
  Effect.map(
    Effect.forEach(
      files,
