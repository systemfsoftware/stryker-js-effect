...
const resultFromExit = (
  ...
  if (exitCode === 0) {
    return {
      status: 'complete',
      tests: [{ id: ALL_TESTS_ID, name: ALL_TESTS_NAME, status: 'success', timeSpentMs }],
    }
  }
  return {
    status: 'complete',
...
const inProcessRunner = (context: TestRunnerBuildContext): Option.Option<InProcessRunnerEffects> =>
  Match.value(context.options.testRunner).pipe(
    Match.when(isCommandRunner, () =>
      Option.some(
        ChildProcessSpawner.ChildProcessSpawner.pipe(Effect.map((spawner) => commandRunner(context, spawner))),
      )),
    Match.when(
      isVmRunner,
      () =>
...
export const buildTestRunner = <ChildRunnerError>(
  context: TestRunnerBuildContext,
...
> =>
  Effect.gen(function*() {
    const inProcess = inProcessRunner(context)
    if (Option.isSome(inProcess)) {
      return yield* inProcess.value
    }
