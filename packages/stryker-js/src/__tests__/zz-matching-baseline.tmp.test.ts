...
const tokenAt = (index: number): string => {
  const tokens = ['*', '**', '?', '.hidden', '/x', '{a,b}', '[a-z]', 'src', 'spec', '!neg', 'a/b'] as const
  const token = tokens[index]
  assert(token !== undefined, `token index ${index} unbound over ${TOKEN_COUNT} tokens`)
  return token
}

const patternTokenArb = Arbitrary.schema(S.Int.check(S.isBetween({ minimum: 0, maximum: TOKEN_COUNT - 1 }))).pipe(
  Arbitrary.map((index) => tokenAt(index)),
)
