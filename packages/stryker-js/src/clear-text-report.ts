import type { Position } from '@systemfsoftware/stryker-js-instrumenter/mutants'
...
function getEmojiForStatus(status: mutants.MutantStatus): string {
  switch (status) {
    case 'Killed':
      return '✅'
    case 'NoCoverage':
...
function mutantLabel(status: mutants.MutantStatus, allowEmojis: boolean): string {
  if (allowEmojis) {
 ...
 */
const REPORT_CHANNEL: Record<mutants.MutantStatus, ReportChannel> = {
  Killed: 'debug',
  Timeout: 'debug',
  RuntimeError: 'debug',
  CompileError: 'debug',
