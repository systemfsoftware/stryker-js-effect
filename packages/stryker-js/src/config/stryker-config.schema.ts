...
const mergeKey = <A = unknown>(
  ...
  Option.match(Option.fromUndefinedOr(overrides[key]), {
    onNone: () => base[key],
    onSome: (override) =>
...
const mergeRecords = <A = unknown>(
  ...
  Record.map(
    { ...Record.filter(base, usable), ...Record.filter(overrides, usable) },
    (_value, key) => mergeKey(base, Record.filter(overrides, usable), key),
  )
