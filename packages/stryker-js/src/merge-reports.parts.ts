...
export const failReason = (reason: string): Effect.Effect<never, MergeReportsFailed> =>
  Effect.gen(function*() {
...
export const writeEncoded = ({ body, raw }: { readonly body: EncodedMerge; readonly raw: MergeCommand }) =>
  Effect.gen(function*() {
    const fs = yield* FileSystem.FileSystem
    const path = yield* Path.Path
