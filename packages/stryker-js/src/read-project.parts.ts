...
export const readProjectCommand = (input: ReadProjectInput) =>
  Effect.gen(function*() {
...
const rangeLawHolds = (
  rangeOf: typeof mutationRangeOf,
  startLine: number,
  endLine: number,
  column: number,
) => {
  ...
  const caseHolds = (pattern: string, startColumn: number, endColumn: number) =>
    JSON.stringify(Option.getOrUndefined(rangeOf(pattern))) ===
...
const exclusionLawHolds = (
  select: typeof selectFiles,
...
const targetLawHolds = (select: typeof selectFiles, files: readonly string[], target: string) => {
  const selected = select({
    inputFileNames: files,
    mutatePatterns: ['**/*'],
...
if (import.meta.vitest !== void 0) {
  ...
  it.prop(
    '∀files_Selection_≡Excluded',
    { of: [FileBatchSchema, GlobSchema, GlobSchema], subject: selectFiles },
    (subject, [files, include, exclude]) => exclusionLawHolds(subject, files, include, exclude),
  )

  it.prop(
    '∀span_Range_≡OneBasedSpan',
    { of: [LineSchema, LineSchema, ColumnSchema], subject: mutationRangeOf },
    (subject, [startLine, endLine, column]) => rangeLawHolds(subject, startLine, endLine, column),
  )

  it.prop(
    '∀files_Selection_≡TargetIntersected',
    { of: [FileBatchSchema, GlobSchema], subject: selectFiles },
    (subject, [files, target]) => targetLawHolds(subject, files, target),
  )
  ...
  it.prop(
    ...
    { of: [FileNameSchema], subject: resolveTestFilesPure },
    (subject, [file]) => {
  ...
  it.prop(
    ...
    { of: [Schema.Array(FileNameSchema)], subject: resolveTestFilesPure },
    (subject, [files]) => {
      const input = files.map(discoveredPath)
      const once = subject(input, VITEST_DEFAULT_TEST_FILE_PATTERNS, DISCOVERY_BASE, DISCOVERY_IGNORES)
      const again = subject(
        [...input, ...once],
        VITEST_DEFAULT_TEST_FILE_PATTERNS,
        DISCOVERY_BASE,
