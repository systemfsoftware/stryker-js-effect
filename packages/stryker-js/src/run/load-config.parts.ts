...
import { isCommandRunner } from '../command-runner.blueprint.js'
import {
...
import { LoadConfigCommand } from './resolve-config.workflow.js'
...
export const describeMessageOf = (message: string): readonly string[] => {
  const state = message
    .split('\n')
...
export const configErrorMessage = (errors: readonly string[]): string =>
  `${configErrorHeadline(errors)} ${errors.join(' ')}`
...
export const readLoadConfig = (input: {
...
export const readRunConfig = (input: {
  readonly cliOptions: Options.PartialStrykerOptions
...
if (import.meta.vitest !== void 0) {
  ...
  it.effect.prop(
    '∀leaf_ValidateOptions_≡WarnsOnlyForNonJsonLeaf',
    { of: [JsonSiblings, S.String, S.Boolean], subject: validateOptions },
    (subject, [siblings, key, asFunction]) =>
      Effect.gen(function*() {
        ...
        return warnings
          .filter((warning) => warning.includes('is not (fully) serializable'))
          .map((warning) => warning.slice(0, expected.length))
          .join('|') === expected
      }),
  )
}
