...
import { createRegistry, planRun } from '../registry.js'
...
const commandOf = (spec: DrainProperty): DrainRegistryCommand =>
  DrainRegistryCommand.make({
...
describe('drainRegistry property tests', () => {
  it.prop(
    '∀d_Decision_≡DrainBrand',
    { of: [DrainPropertySpec], subject: drainRegistry },
    (drain, [spec]) => {
      const result = drain(commandOf(spec))
  ...
  it.prop(
    '∀c_DrainCommand_≡ExpectedOutcome',
    { of: [DrainPropertySpec], subject: drainRegistry },
    (drain, [spec]) => {
      const result = drain(commandOf(spec))
...
describe('planRun property tests', () => {
  it.prop(
    '∀reg_PlanRun_≡PlanRun',
    { of: [RegistrySpecSchema], subject: planRun },
    (plan, [specs]) => {
      const registry = registryOfSpecs(specs)
      const plan1 = plan(registry)
      const plan2 = plan(registry)
  ...
  it.prop(
    '∀reg_PlanRun_∈UniqueNames',
    { of: [RegistrySpecSchema], subject: planRun },
    (plan, [specs]) => {
      const planned = plan(registryOfSpecs(specs))
      const fullNames = planned.map((p) => p.fullName)
