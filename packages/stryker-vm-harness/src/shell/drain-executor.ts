...
import { type FixtureProps, usedFixtureProps } from '../core/fixture-props.js'
import {
  cleanupAll,
  cleanupArrayOf,
  cleanupCountOf,
...
import { decideTestStatus } from '../core/test-status.js'
...
const fireHook = (
  ...
  const ran = Effect.promise(() =>
    promise.then(
      (value) => ({ value: isCleanup(value) ? value : undefined, failure: undefined }) as FireOutcome,
      <A = unknown>(cause: A) => ({ value: undefined, failure: attemptFailureOf(messageOf(cause)) }) as FireOutcome,
    )
  )
  return Effect.map(
    Effect.result(timeoutOf(ran, timeoutMs, () => {
      detach(promise)
...
const resolveDepsOf = (
  ...
  const pending: FixtureDefinition[] = []
      ...
      Effect.gen(function*() {
        ...
        const hasRunnableChild = (node: RunnerNode): boolean =>
          node.planned !== undefined
            ...
            : node.children.some(hasRunnableChild)
        ...
        let workerContext: object = Object.create(null)
        ...
        const runBodyFor = (planned: PlannedTest, timeoutMs: number): Effect.Effect<AttemptFailure | undefined> => {
          ...
          const promise = Promise.resolve().then(() => planned.test.fn?.(context) as AttemptFailure | undefined)
          const ran = Effect.promise(() =>
            promise.then(
              () => undefined,
              <A = unknown>(cause: A) =>
        ...
        const runAttempt = (
          ...
          const runChainAt = (index: number): Effect.Effect<AttemptFailure | undefined> => {
            ...
            return Effect.gen(function*() {
              const failure = yield* runAroundHook(
                registration,
                context,
                config.hookTimeout,
                'runTest()',
        ...
        const suiteHooksAfterEach = (planned: PlannedTest): ReadonlyArray<RegisteredHook> => {
          ...
          return reversed.flatMap((id) => afterHooksFor(config, registry.suiteHooks.get(id)?.afterEach ?? [])).concat(
            afterHooksFor(config, registry.rootHooks.get(planned.test.file)?.afterEach ?? []),
          )
        }
        ...
        const runAttemptCycle = (
          ...
          const task = planned.test.task
          const aroundChain = [
            ...planned.chain.flatMap((id) => registry.suiteAround.get(id)?.aroundEach ?? []),
            ...(registry.rootAround.get(planned.test.file)?.aroundEach ?? []),
          ]
          return Effect.gen(function*() {
        ...
        const invertIfNeeded = (
          ...
          const syntaxFailure = (task.result?.errors ?? []).some((error) => error.name === 'TestSyntaxError')
          if (syntaxFailure) {
            return failure
          }
        ...
        const asHookContext = (target: object): HarnessTestContext => target as HarnessTestContext
        ...
        const runChildrenNodes = (node: RunnerNode, file: string, fileContext: object): Effect.Effect<void> =>
          Effect.gen(function*() {
            ...
            const runWithCeiling = (
              pending: ReadonlyArray<RunnerNode>,
              remaining: number,
            ): Effect.Effect<void> =>
              remaining <= 0 || pending.length === 0
                ? Effect.forEach(pending, (child) => runSuiteNode(child, file, fileContext), { discard: true })
                : Effect.gen(function*() {
                  const head = pending.slice(0, remaining)
                  const tail = pending.slice(remaining)
                  yield* Effect.forEach(
                    head,
                    (child) => Effect.flatMap(Effect.yieldNow, () => runSuiteNode(child, file, fileContext)),
            ...
            const flush = (concurrent: boolean, pending: ReadonlyArray<RunnerNode>): Effect.Effect<void> => {
              ...
              if (!concurrent) {
                return Effect.forEach(pending, (child) => runSuiteNode(child, file, fileContext), { discard: true })
              }
        ...
        try {
          ...
          const guardedRun = timeoutMs !== undefined
            ? Effect.map(Effect.timeoutOption(runExecution, timeoutMs), (option) => Option.getOrUndefined(option))
            : Effect.map(runExecution, (value) => value)
