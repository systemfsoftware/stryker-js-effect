#!/usr/bin/env -S deno run --config=scripts/deno.json --allow-run=git,corepack,pnpm,npm --allow-read --allow-write --allow-env=NPM_REGISTRY,GITHUB_REPOSITORY --allow-net=registry.npmjs.org
import { pooledMap } from '@std/async/pool'
import { parseArgs } from '@std/cli/parse-args'
import { join } from '@std/path'
import { queryRegistry, type RegistrySnapshot } from './lib/npm-query.ts'
import { expectedSlug, publicWorkspacePackages, WORKFLOW_FILE, type WorkspacePackage } from './lib/oidc.ts'
