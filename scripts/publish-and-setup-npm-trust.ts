#!/usr/bin/env -S deno run --config=scripts/deno.json --allow-run=git,pnpm,npm --allow-read --allow-write --allow-env=NPM_REGISTRY,GITHUB_REPOSITORY --allow-net=registry.npmjs.org

import { pooledMap } from '@std/async/pool'
import { parseArgs } from '@std/cli/parse-args'
import { queryRegistry } from './lib/npm-query.ts'
import { expectedSlug, publicWorkspacePackages, WORKFLOW_FILE } from './lib/oidc.ts'
...
async function reconcileTrust(pkgName: string, cwd: string, dryRun: boolean): Promise<boolean> {
  ...
  if (existing.some(matchesTarget)) {
    const stale = existing.filter((cfg) => !matchesTarget(cfg))
    for (const cfg of stale) {
      if (!cfg.id) continue
      console.log(`  Cleaning up stale config ${cfg.id} (${cfg.repository ?? 'other repo'})`)
      console.log(`  > npm trust revoke ${pkgName} --id=${cfg.id}`)
      if (!dryRun) {
...
async function publishAndTrust(p: Owed): Promise<{ name: string; ok: boolean }> {
  ...
  if (p.mode === 'debut') {
    if (await hasBuildScript(p.dir)) {
      console.log(`  > pnpm --filter ${name} build`)
      if (!dryRun && !(await runInteractive(['pnpm', '--filter', name, 'build'], p.dir)).success) {
        return { name, ok: false }
      }
    }
    console.log(`  > pnpm --filter ${name} publish --access public --no-git-checks`)
    if (
      !dryRun &&
      !(await runInteractive(['pnpm', '--filter', name, 'publish', '--access', 'public', '--no-git-checks'], p.dir))
        .success
    ) {
      return { name, ok: false }
    }
  ...
  const trustOk = await reconcileTrust(name, p.dir, dryRun)
  if (!trustOk) return { name, ok: false }
