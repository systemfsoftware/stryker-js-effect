...
| **E2E-2** | Fixture oracles are authored literals in the journey that asserts them, or in the `tests/__fixtures__/*.fixture.ts` verify helper that journey asserts through: a run may confirm the numbers, never originate them. A mismatch is triaged as a fixture-authoring error or a product bug, never auto-copied into the oracle.                                                                                                                                                                                                              | `review` — every expected count appears as an authored literal in `tests/*.e2e.test.ts` or in the `tests/__fixtures__/*.fixture.ts` verify helper it asserts through (CONST-T10)             |
