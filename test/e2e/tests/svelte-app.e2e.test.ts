...
const stepVerifyFormats = (expect: ExpectStatic, events: ReadonlyArray<RunEvent>): void => {
  const formats = events.find((event): event is Extract<RunEvent, { _tag: 'formats' }> => event._tag === 'formats')
...
const stepVerifyNoSvelteSkipped = (expect: ExpectStatic, events: ReadonlyArray<RunEvent>): void => {
  const skipped = events.find((event): event is Extract<RunEvent, { _tag: 'skipped' }> => event._tag === 'skipped')
  expect.soft(skipped?.files.filter((file) => file.file.endsWith('.svelte')) ?? []).toEqual([])
}

const stepVerifyMutants = (expect: ExpectStatic, events: ReadonlyArray<RunEvent>): void => {
